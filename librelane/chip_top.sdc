...
# Set max transition constraint for sparse/large layout (prevents buffer explosion)
set_max_transition 8.0 [current_design]

set has_cts 0
if { [llength [get_cells -hierarchical -filter {ref_name =~ *clkbuf*}]] > 0 } {
    set has_cts 1
}

if { $has_cts } {
    puts {[INFO] Clock tree buffers found. Setting propagated clock.}
    set_propagated_clock [all_clocks]
} else {
    puts {[INFO] No clock tree buffers found. Setting ideal clock.}
